...
export interface FrameStepHandle {
  handleAdvance: () => Promise<void>
}

export const FrameStep = forwardRef<FrameStepHandle, { onNext: () => void }>(function FrameStep({ onNext }, ref) {
  ...
  const useMagicSelectionAsFrame = async () => {
    ...
    if (!bounds || !project.template) { toast.error("Belum ada area dipilih."); return }
  ...
  advanceRef.current = useMagicSelectionAsFrame

  useImperativeHandle(ref, () => ({
    handleAdvance: async () => {
      if (magic.state.active && magic.state.mask) {
        await advanceRef.current()
      } else {
        onNext()
      }
    }
  }), [magic.state.active, magic.state.mask, onNext])

  const change = (patch: Partial<PhotoFrame>) => {
    if (!project.template || !frame) return
    ...
    <section>
      ...
      <div className="frame-layout">
        ...
        <div className="settings">
          {magic.state.active ? (
            <>
              <button className={`button full magic-trigger picker-active`} aria-pressed={true} onClick={magic.deactivate}>
                <WandSparkles /> Tutup Magic Select
              </button>
              <MagicSelectToolbar state={magic.state} onMode={magic.setMode} onTolerance={magic.setTolerance} onFeather={magic.setFeather} onInvert={magic.invert} onExpand={magic.expand} onContract={magic.contract} onReset={magic.reset} onCancel={magic.cancel} onClose={magic.deactivate} />
            </>
          ) : (
            <>
              ...
              <button className="button full magic-trigger secondary" onClick={() => void magic.activate()}>
                <WandSparkles /> Magic Select
              </button>
            </>
          )}
        </div>
